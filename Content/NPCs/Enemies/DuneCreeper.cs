    ...
    class DuneCreeper : ModNPC
    {
                    ...
                    if (Main.netMode != 1)
                    {
                        if (npc.type == 292)
                        {
                            for (int num234 = 0; num234 < 4; num234++)
                            {
                                ...
                                num247 *= num240;
                                num245 *= num240;
                                Projectile.NewProjectile(value9.X, value9.Y, num247, num245, num235, num236, 0f, Main.myPlayer, 0f, 0f);
                            }
                        }
                        else if (npc.type == 411)
                        {
                            Projectile.NewProjectile(value9.X, value9.Y, num247, num245, num235, num236, 0f, Main.myPlayer, 0f, (float)npc.whoAmI);
                        }
                        else if (npc.type == 424)
                        {
                            for (int num227 = 0; num227 < 4; num227++)
                            {
                                Projectile.NewProjectile(npc.Center.X - (float)(npc.spriteDirection * 4), npc.Center.Y + 6f, (float)(-3 + 2 * num227) * 0.15f, (0f - (float)Main.rand.Next(0, 3)) * 0.2f - 0.1f, num235, num236, 0f, Main.myPlayer, 0f, (float)npc.whoAmI);
                            }
                        }
        ...
        IL_8a60:
            if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
            {
                ...
                Projectile.NewProjectile(vector31.X, vector31.Y, num286, num285, num277, num278, 0f, Main.myPlayer, 0f, 0f);
                npc.ai[2] = 0f;
            }
            goto IL_8c30;
        IL_8c30:
        ...
        IL_8d96:
            if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
            {
                ...
                Projectile.NewProjectile(vector32.X, vector32.Y, num273, num272, num264, num265, 0f, Main.myPlayer, 0f, 0f);
                npc.ai[2] = 0f;
            }
            goto IL_8f61;
        IL_41d5:
        ...
        IL_6bf9:
                ...
                if (Main.netMode != 1)
                {
                    ...
                    if (npc.localAI[2] >= (float)(360 + Main.rand.Next(360)) && npc.Distance(Main.player[npc.target].Center) < 400f && Math.Abs(npc.DirectionTo(Main.player[npc.target].Center).Y) < 0.5f && Collision.CanHitLine(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
                    {
                        ...
                        for (int num318 = 0; num318 < 4; num318++)
                        {
                            ...
                            Projectile.NewProjectile(vector29.X, vector29.Y, vector27.X, vector27.Y, 577, num319, 1f, Main.myPlayer, 0f, 0f);
                        }
                    }
                }
            }
            ...
            if (Main.netMode != 1 && Main.expertMode && npc.target >= 0 && (npc.type == 163 || npc.type == 238) && Collision.CanHit(npc.Center, 1, 1, Main.player[npc.target].Center, 1, 1))
            {
                ...
                if (npc.localAI[0] > (float)Main.rand.Next(180, 900))
                {
                    ...
                    vector30.Normalize();
                    vector30 *= 8f;
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, vector30.X, vector30.Y, 472, 18, 0f, Main.myPlayer, 0f, 0f);
                }
            }
        ...
        IL_aba2:
            if (npc.type == 109 && Main.netMode != 1 && !Main.player[npc.target].dead)
            {
                ...
                if (npc.ai[2] > 450f)
                {
                    ...
                    int num211 = -5;
                    int num210 = Projectile.NewProjectile(vector21.X, vector21.Y, (float)num212, (float)num211, 75, 0, 0f, Main.myPlayer, 0f, 0f);
