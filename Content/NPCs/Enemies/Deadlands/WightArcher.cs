...
namespace ExoriumMod.Content.NPCs.Enemies.Deadlands
{
    class WightArcher : ModNPC
    {
        ...
        {
            if (spawnInfo.Player.InModBiome<DeadlandBiome>() && spawnInfo.SpawnTileType == ModContent.TileType<Content.Tiles.AshenDustTile>()) return 0.14f;
            return 0;
        }
            ...
            if (NPC.type == 389)
            {
                ...
                else
                {
                    ...
                    if (NPC.ai[2] == 20f && Main.netMode != 1)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y + 8f, (float)(NPC.direction * 6), 0f, ProjectileID.WoodenArrowHostile, damage, 1f, Main.myPlayer, 0f, 0f);
                    }
                }
            }
            if (false && NPC.type != 111 && NPC.type != 206 && NPC.type != 214 && NPC.type != 215 && NPC.type != 216 && NPC.type != 290 && NPC.type != 291 && NPC.type != 292 && NPC.type != 293 && NPC.type != 350 && NPC.type != 379 && NPC.type != 380 && NPC.type != 381 && NPC.type != 382 && (NPC.type < 449 || NPC.type > 452) && NPC.type != 468 && NPC.type != 481 && NPC.type != 411 && NPC.type != 409 && (NPC.type < 498 || NPC.type > 506) && NPC.type != 424 && NPC.type != 426 && NPC.type != 520)
                    ...
                    if (Main.netMode != 1)
                    {
                        if (NPC.type == 292)
                        {
                            for (int num234 = 0; num234 < 4; num234++)
                            {
                                ...
                                num247 *= num240;
                                num245 *= num240;
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), value9.X, value9.Y, num247, num245, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, 0f);
                            }
                        }
                        else if (NPC.type == 411)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromAI(), value9.X, value9.Y, num247, num245, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, (float)NPC.whoAmI);
                        }
                        else if (NPC.type == 424)
                        {
                            for (int num227 = 0; num227 < 4; num227++)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X - (float)(NPC.spriteDirection * 4), NPC.Center.Y + 6f, (float)(-3 + 2 * num227) * 0.15f, (0f - (float)Main.rand.Next(0, 3)) * 0.2f - 0.1f, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, (float)NPC.whoAmI);
                            }
                        }
                        else
                        {
            ...
            if (NPC.type == 463 && Main.netMode != 1)
            {
                ...
                if (NPC.justHit && NPC.localAI[3] <= 0f && Main.rand.Next(3) == 0)
                {
                    ...
                    for (int num362 = 0; num362 < num370; num362++)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.position.Y + (float)(NPC.width / 4), value7.X, value7.Y, ProjectileID.WoodenArrowHostile, damage, 1f, 255, 0f, 0f);
                    }
                }
            }
        ...
        IL_8a60:
            if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_FromAI(), vector31.X, vector31.Y, num286, num285, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, 0f);
                NPC.ai[2] = 0f;
            }
            goto IL_8c30;
        IL_8c30:
        ...
        IL_8d96:
            if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_FromAI(), vector32.X, vector32.Y, num273, num272, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, 0f);
                NPC.ai[2] = 0f;
            }
            goto IL_8f61;
        IL_41d5:
        ...
        IL_6bf9:
                ...
                if (Main.netMode != 1)
                {
                    ...
                    if (NPC.localAI[2] >= (float)(360 + Main.rand.Next(360)) && NPC.Distance(Main.player[NPC.target].Center) < 400f && Math.Abs(NPC.DirectionTo(Main.player[NPC.target].Center).Y) < 0.5f && Collision.CanHitLine(NPC.Center, 0, 0, Main.player[NPC.target].Center, 0, 0))
                    {
                        ...
                        for (int num318 = 0; num318 < 4; num318++)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromAI(), vector29.X, vector29.Y, vector27.X, vector27.Y, ProjectileID.WoodenArrowHostile, damage, 1f, Main.myPlayer, 0f, 0f);
                        }
                    }
                }
            }
            ...
            if (Main.netMode != 1 && Main.expertMode && NPC.target >= 0 && (NPC.type == 163 || NPC.type == 238) && Collision.CanHit(NPC.Center, 1, 1, Main.player[NPC.target].Center, 1, 1))
            {
                ...
                if (NPC.localAI[0] > (float)Main.rand.Next(180, 900))
                {
                    ...
                    vector30.Normalize();
                    vector30 *= 8f;
                    Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y, vector30.X, vector30.Y, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, 0f);
                }
            }
        ...
        IL_aba2:
            if (NPC.type == 109 && Main.netMode != 1 && !Main.player[NPC.target].dead)
            {
                ...
                if (NPC.ai[2] > 450f)
                {
                    ...
                    int num211 = -5;
                    int num210 = Projectile.NewProjectile(NPC.GetSource_FromAI(), vector21.X, vector21.Y, (float)num212, (float)num211, ProjectileID.WoodenArrowHostile, damage, 0f, Main.myPlayer, 0f, 0f);
