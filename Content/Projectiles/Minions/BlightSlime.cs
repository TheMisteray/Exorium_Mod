...
namespace ExoriumMod.Content.Projectiles.Minions
{
    class BlightSlime : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			{
				player.ClearBuff(BuffType<Buffs.Minions.BlightSlime>());
			}
			if (player.HasBuff(BuffType<Buffs.Minions.BlightSlime>()))
			{
					...
					if (Projectile.type != 198 && Projectile.type != 380)
					{
						if (Projectile.type == 211)
						{
							...
							if (Projectile.localAI[0] == 1f)
							{
								...
								{
									//LOOK - may be how to have lighting affect shaders
									//int num146 = Dust.NewDust(new Vector2(projectile.position.X + 3f, projectile.position.Y + 4f), 14, 14, DustType<Dusts.Rainbow>(), 0f, 0f, 0, default(Color), 1f);
						...
						if (Projectile.ai[1] == 0f)
						{
							...
							if (flag9 || true || (Projectile.type >= 390 && Projectile.type <= 392))
							{
								...
								if (true && Projectile.localAI[0] > 0f)
								{
									num133 += 100;
								}
			...
			else
			{
				if (flag9)
				{
					...
					else
					{
						...
						else if (num102 >= 0 && num104 < 800f + num109)
						{
							...
							else if (Projectile.owner == Main.myPlayer)
							{
								...
								int num83 = 195;
								int num82 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), vector7.X, vector7.Y, num92, num89, num83, num84, Projectile.knockBack, Main.myPlayer, 0f, 0f);
								Main.projectile[num82].timeLeft = 300;
								if (num92 < 0f)
								{
									Projectile.direction = -1;
								}
