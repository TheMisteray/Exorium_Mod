...
float4 PixelShaderFunction(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    color.g -= tex2D(samplerTex2, st2).x * 1.5;
    color.g += tex2D(samplerTex2, st3).x * 3;

    color.b -= tex2D(samplerTex2, st2).x * 3;
    color.b += tex2D(samplerTex2, st3).x * 1.5;

    color.r += tex2D(samplerTex2, st2).x * 1;
    color.r += tex2D(samplerTex2, st3).x * 1.5;
